    ...
    <div style={{ background: '#F4F6FA', minHeight: '100vh', fontFamily: "'Inter', -apple-system, sans-serif" }}>
      ...
      <div className="report-page" style={{ maxWidth: 900, margin: '0 auto', padding: '32px 20px 64px' }}>
        ...
        <div className="fade" style={{
          background: `linear-gradient(135deg, #0F172A 0%, ${agencyBranding?.accentColor || '#1B4F8A'} 100%)`,
          ...
          <div style={{ position: 'relative' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 28 }}>
              {agencyBranding?.logoUrl ? (
                <img src={agencyBranding.logoUrl} style={{ height: 32, maxWidth: 140, objectFit: 'contain' }} alt={agencyBranding.name} />
              ) : (
                <OOHLogo white />
              )}
              ...
              <span style={{ fontSize: '0.6875rem', fontWeight: 600, color: 'rgba(255,255,255,0.45)', textTransform: 'uppercase', letterSpacing: '0.1em' }}>
                {agencyBranding?.tagline || 'Campaign Performance Report'}
              </span>
            </div>
        ...
        <div style={{ textAlign: 'center', padding: '20px 0 0', borderTop: '1px solid #E8EDF2', marginTop: 8 }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, marginBottom: 6 }}>
            {agencyBranding?.logoUrl
              ? <img src={agencyBranding.logoUrl} style={{ height: 24, maxWidth: 100, objectFit: 'contain', opacity: 0.5 }} alt={agencyBranding.name} />
              : <OOHLogo dark />
            }
          </div>
          {agencyBranding ? (
            <p style={{ fontSize: '0.6875rem', color: '#CBD5E1', margin: '0 0 2px' }}>
              Prepared by {agencyBranding.name}{agencyBranding.website ? ` · ${agencyBranding.website}` : ''} · Powered by OOH Platform
            </p>
          ) : (
            <p style={{ fontSize: '0.6875rem', color: '#CBD5E1', margin: '0 0 2px' }}>
