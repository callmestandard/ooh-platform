...
async function fetchImageAsBase64(url: string): Promise<{ data: string; ext: string } | null> {
  try {
    const res = await fetch(url, { signal: AbortSignal.timeout(5000) });
    if (!res.ok) return null;
    ...
    return { data: `data:${contentType};base64,${base64}`, ext };
  } catch {
    return null;
  }
}
...
async function generatePDF(campaign: any, bookings: any[], compByBooking: Record<string, any>, brand: any, generatedAt: string): Promise<Buffer> {
  // Pre-fetch all photos before the stream starts — PDFKit image() is synchronous
  const photoBuffers: Record<string, Buffer | null> = {};
  await Promise.all(
    bookings.map(async (booking) => {
      const comp = compByBooking[booking.id];
      if (comp?.photo_url) {
        photoBuffers[booking.id] = await fetchImageBuffer(comp.photo_url);
      }
    })
  );
  ...
  return new Promise((resolve, reject) => {
    ...
    bookings.forEach((booking, idx) => {
      ...
      const photoBuf = photoBuffers[booking.id] ?? null;
      ...
      if (photoBuf) {
        // Render the real photo, fitted and centred within the photo box
        try {
          ...
          // Thin border overlay so the box feels intentional
          doc.rect(photoX, photoY, photoW, photoH)
            .stroke('#E2E8F0').strokeOpacity(0.6).lineWidth(1);
        } catch {
          // Corrupt/unsupported image — graceful fallback
          doc.rect(photoX, photoY, photoW, photoH).fill('#F1F5F9').fillOpacity(1);
          doc.fontSize(10).font('Helvetica').fillColor('#CBD5E1').fillOpacity(1)
            .text('Photo unavailable', photoX, photoY + photoH / 2 - 8, { width: photoW, align: 'center' });
        }
      } else if (comp?.photo_url) {
        // URL stored but fetch failed — show a gentle note rather than a broken box
        doc.rect(photoX, photoY, photoW, photoH).fill('#F8FAFC').fillOpacity(1)
          .rect(photoX, photoY, photoW, photoH).stroke('#E2E8F0').strokeOpacity(1).lineWidth(1);
        doc.fontSize(11).font('Helvetica').fillColor('#94A3B8').fillOpacity(1)
          .text('Photo submitted — preview unavailable', photoX, photoY + photoH / 2 - 10, { width: photoW, align: 'center' });
      } else {
        doc.rect(photoX, photoY, photoW, photoH).fill('#F8FAFC').fillOpacity(1)
          .rect(photoX, photoY, photoW, photoH).stroke('#E2E8F0').strokeOpacity(1).lineWidth(1);
        doc.fontSize(13).font('Helvetica').fillColor('#CBD5E1').fillOpacity(1)
          .text('No photo submitted yet', photoX, photoY + photoH / 2 - 10, { width: photoW, align: 'center' });
      }
...
async function generatePPTX(campaign: any, bookings: any[], compByBooking: Record<string, any>, brand: any, generatedAt: string): Promise<Buffer> {
  ...
  cover.addText(brand.logo_text, {
    ...
    fontSize: 72, bold: true, color: primaryHex, fontFace: 'Calibri',
  });
  ...
  // ── Board slides ──
  // Fetch photos in parallel before building slides
  const photoBase64Map: Record<string, { data: string; ext: string } | null> = {};
  await Promise.all(
    bookings.map(async (booking) => {
      const comp = compByBooking[booking.id];
      if (comp?.photo_url) {
        photoBase64Map[booking.id] = await fetchImageAsBase64(comp.photo_url);
      }
    })
  );
  ...
  bookings.forEach((booking, idx) => {
    ...
    const board = booking.boards;
    const hasComp = !!comp;
    const photo = photoBase64Map[booking.id] ?? null;
    ...
    // Board number circle
    slide.addShape(pptx.ShapeType.ellipse, { x: 0.3, y: 0.2, w: 0.7, h: 0.7, fill: { color: primaryHex } });
    ...
    if (photo) {
      // Real photo — embed as base64 data URI, contained within the photo box
      try {
        slide.addImage({
          data: photo.data,
          x: photoX,
      ...
      slide.addText('Photo submitted — preview unavailable', {
        x: photoX, y: photoY + photoH / 2 - 0.2, w: photoW, h: 0.4,
        fontSize: 12, color: '94A3B8', align: 'center', fontFace: 'Calibri',
      });
    } else {
