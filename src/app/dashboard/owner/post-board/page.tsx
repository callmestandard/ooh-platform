...
function PostBoardContent() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    // Notify the owner that their board is live
    await createNotification({
      recipientRole: 'owner',
      type: 'new_booking',
      title: `Your board "${boardName.trim()}" is now live`,
      body: `Agencies in ${city} can now discover and make offers on your listing.`,
      link: '/dashboard/owner',
    });

    // Notify agencies that new inventory is available in this city
    await createNotification({
      recipientRole: 'agency',
      type: 'new_booking',
      title: `New board listed in ${city}`,
      body: `${boardName.trim()} — ${FORMAT_OPTIONS.find(f => f.value === format)?.label} · ${formatNaira(parseFloat(askingRate))}/month asking rate`,
      link: '/dashboard/agency/marketplace',
    });
  ...
  if (done) {
    return (
      <div style={{ maxWidth: 560, margin: '0 auto', textAlign: 'center', padding: '3rem 0' }}>
        ...
        {/* Share your listing */}
        <div style={{ background: '#F8FAFC', border: '1px solid #E8EDF2', borderRadius: 12, padding: '16px 20px', marginBottom: 20, textAlign: 'left' }}>
          <p style={{ fontSize: '0.75rem', fontWeight: 700, color: '#374151', margin: '0 0 10px', textTransform: 'uppercase', letterSpacing: '0.06em' }}>Share your listing</p>
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            <a
              href={`https://wa.me/?text=${encodeURIComponent(`Hi! I just listed my ${FORMAT_OPTIONS.find(f => f.value === format)?.label} in ${city} on OOH Platform. ${boardName} — asking ${formatNaira(parseFloat(askingRate || '0'))}/month. View it here: ${typeof window !== 'undefined' ? window.location.origin : ''}/marketplace`)}`}
              target="_blank"
              rel="noopener noreferrer"
              style={{ display: 'flex', alignItems: 'center', gap: 7, padding: '10px 16px', borderRadius: 9, background: '#25D366', color: '#fff', textDecoration: 'none', fontSize: '0.875rem', fontWeight: 700, fontFamily: 'inherit' }}
            >
            ...
            <button
              onClick={() => { navigator.clipboard.writeText(`${typeof window !== 'undefined' ? window.location.origin : ''}/marketplace`); }}
              style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '10px 16px', borderRadius: 9, background: '#fff', border: '1.5px solid #E2E8F0', color: '#374151', fontSize: '0.875rem', fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' }}
            >
          ...
          <p style={{ fontSize: '0.6875rem', color: '#94A3B8', margin: '8px 0 0' }}>
            Send this to agencies directly — they can browse your board and make an offer on the platform
          </p>
        </div>
